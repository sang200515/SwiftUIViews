...
private struct Scope_WithAnimation_OnSingleView: View {
    ...
    var body: some View {
        VStack(spacing: 20) {
            ...
            BannerText("Animating from a state variable is very powerful because it allows you to animate a single property, independently of other property animations.'",
                       backColor: .yellow, textColor: .black)
            
            HStack(spacing: 50) {
                Button(action: {
                    withAnimation { flipHorizontally.toggle() }
                }) {
                    Image(systemName: "flip.horizontal.fill").font(.system(size: 50)).padding()
                }
                
                Button(action: {
                    withAnimation { flipVertically.toggle() }
                }) {
