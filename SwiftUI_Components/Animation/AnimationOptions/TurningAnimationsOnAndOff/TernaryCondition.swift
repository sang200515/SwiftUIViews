...
fileprivate struct TernaryCondition: View {
    ...
    var body: some View {
        VStack(spacing: 20) {
            ...
            Button(action: {
                showCard.toggle()
                useAnimation = true // Turn on animations
            }) {
            ...
            VStack {
                HStack {
                    ...
                    Spacer()
                    Button(action: {
                        useAnimation = false // Turn off animations
                        showCard.toggle()
                    }) {
            ...
            .animation(useAnimation ? .default : .none, value: showCard) // Use a ternary operator here
        }.font(.title)
    }
}
